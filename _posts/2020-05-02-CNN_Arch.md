...
To implement this, we do X + F(X) in each block. 
  <img src="https://github.com/sportsunrahul/sportsunrahul.github.io/blob/master/images/cnn/resnet1.PNG?raw=true" alt="Photo" style="width: 450px;"/> 

We might face an issue where the dimensions of X and F(X) don’t match. We do linear projections to fix this.
	1. If the channels don’t match, we do 1x1 on X to match them.
	2. If the width, height doesn’t match, we do stride on X using 1x1.
  
Identity Mappings in Resnets
Propagation formulations behind the residual building blocks suggests that the forward and backward signals can be directly propagated from one block to any other block, when using identity mappings as the skip connections and after-addition activation.
